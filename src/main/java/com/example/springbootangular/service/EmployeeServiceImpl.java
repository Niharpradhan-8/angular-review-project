...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public ApiResponseDto<Employee> getEmployeeById(EmployeePk employeePk) {
        try {
            ...
            log.info("Employee retrieved successfully with id {}",employeePk);
            return new ApiResponseDto<>("SUCCESS", messageSource.getMessage("EMPLOYEE.MSG.VALERR0003",null, LocaleContextHolder.getLocale()), employee);
        } catch (Exception e) {
            log.error(e.getMessage());
            throw new RecordNotFoundException(e.getMessage());
        }
